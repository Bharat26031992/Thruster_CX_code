...
Lx = 6; Ly = 3; dx = 0.006; dy = 0.006;

% Telemetry and 3D objects
iter_history         = []; ebs_history     = []; div_history = [];
divergence_snapshots = {}; recorded_frames = {};
...
plasmaContainer = uipanel(plasmaScroll, 'Position', [0, 0, 260, 460], ...
    'BackgroundColor', [0.98 0.98 0.99], 'BorderType', 'none');
...
ui_ne = uieditfield(plasmaContainer, 'numeric', 'Position',...
    [fieldX, y0-3*drow, fieldW, rowH], 'Value', 2e17);
...
ui_n0 = uieditfield(plasmaContainer, 'numeric', 'Position', ...
    [fieldX, y0-4*drow, fieldW, rowH], 'Value', 4e18);
...
btn_saveGif = uibutton(plasmaContainer, 'Text', 'Save GIF Animation', ...
    'Position', [10, 30, 240, 22], 'BackgroundColor', [0.9 0.8 1], ...
    'ButtonPushedFcn', @saveGif);

chk_erosion = uicheckbox(plasmaContainer, 'Text', 'Enable erosion simulation', ...
    'Position', [10, 0, 180, 22], 'Value', true, 'FontWeight', 'bold', ...
    'FontColor', [0.6 0 0]);

%% --- Default two-grid configuration
grid_defs(1) = struct('name', 'Screen', 'voltage', 1200, 'thickness', 0.6, ...
    'holeRadius', 1.1, 'chamfer', -8, 'gapAfter', 1.0);
grid_defs(2) = struct('name', 'Accel', 'voltage', -200, 'thickness', 0.5, ...
    'holeRadius', 0.9, 'chamfer', 0, 'gapAfter', 0.5);
grid_defs(3) = struct('name', 'Accel', 'voltage', 50, 'thickness', 0.5, ...
    'holeRadius', 1.1, 'chamfer', 0, 'gapAfter', 0.0);
rebuildGridUI();
...
while ishandle(fig)
    if sim.isRunning && ~isempty(Ex)
        ...
        % Minimum occupancy target: 4 particles per radial mesh cell
        yMinInject = 0.05;
        ...
        % --- Particle displacement check: vmax*dt < Dl_min ---
        ...
        % Force all particle arrays and masks to be column vectors
        ...
        nPart = numel(p_x);
        if numel(p_y) ~= nPart || numel(p_vx) ~= nPart || numel(p_vy) ~= nPart || numel(p_isCEX) ~= nPart
            error('Particle arrays have inconsistent sizes before filtering.');
        end
        if numel(hit_grid) ~= nPart || numel(out_of_bounds) ~= nPart
            error('Mask size does not match particle array size before filtering.');
        end
        ...
        primary_mask = ~p_isCEX & (p_x > 0.2); % Where CEX starts to be created
        if any(primary_mask)
        ...
        if mod(iteration, 5) == 0
            ...
            if any(beam_mask) % coloring the ions differently depending on the energy
                set(h_beam, ...
                    'XData', p_x(beam_mask), ...
                    'YData', p_y(beam_mask), ...
                    'CData', KE_eV(beam_mask));
            else
            ...
            if any(cex_mask)
                set(h_cex, ...
                    'XData', p_x(cex_mask), 'YData', p_y(cex_mask));
                    %'CData', KE_eV(cex_mask)); % Colors updated with sidebar
                
            else
                %set(h_cex, 'XData', NaN, 'YData', NaN, 'CData', NaN);
            ...
            if ~chk_erosion.Value
                % --- Visualization-only sputter map ---
                ...
                if any(visual_mask, 'all')
                    ...
                    colormap(ax_damage, sputterMap);   % black->red->orange->yellow
                ...
                unhit_mask = isBound & (damage_map <= 0);
                [gy, gx] = find(unhit_mask);
                scatter(ax_damage, (gx-1)*dx, (gy-1)*dy, 2, [0.25 0.25 0.25], ...
                    'filled', 'MarkerFaceAlpha', 0.20);
                hit_mask_vis = isBound & (damage_map > 0);
                [gyh, gxh] = find(hit_mask_vis);
                
                if ~isempty(gxh)
                    scatter(ax_damage, (gxh-1)*dx, (gyh-1)*dy, 8, damage_map(hit_mask_vis), ...
                        'filled', 'MarkerEdgeColor', 'none');
                end
                colormap(ax_damage, sputterMap);
            else
                % --- Existing erosion-mode rendering ---
...
function toggleSim(~, ~)
% Starts, pauses, or resumes the beam extraction loop.
% It also prevents execution before the electrostatic domain is built.
    if isempty(Ex)
