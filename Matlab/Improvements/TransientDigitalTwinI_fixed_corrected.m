...
Lx = 4.5; Ly = 2.8; dx = 0.006; dy = 0.006;
...
%% Electric field diagnostic variables

axialEnergySnapshots = {};
axialDiagBinCenters = [];
...
%% --- Initial conditions
y0     = 440;  drow   = 28;   labelX = 10;  fieldX = 150;
...
function doOneSimulationStep()
    if sim.stopRequested || ~sim.isRunning || sim.shouldClose
        sim.isRunning = false;
        return
    end
    ...
    yCenter = Ly/2;
    yMinInject = max(0.05, yCenter - sourceRadius);
    yMaxInject = min(Ly - 0.05, yCenter + sourceRadius);
    nCellsInject = max(1, ceil((yMaxInject - yMinInject)/dy));
    minParticlesPerCell = 4;
    numInjectBase = 30;
    numinject = max(numInjectBase, minParticlesPerCell * nCellsInject);
    
    new_y = linspace(yMinInject, yMaxInject, numinject).' + (rand(numinject,1)-0.5)*0.05;
    new_y = min(max(new_y, yMinInject), yMaxInject);
    new_x = repmat(0.1, numinject, 1);
    ...
    p_x = [p_x; new_x];
    p_y = [p_y; new_y];
    p_vx = [p_vx; repmat(v_bohm, numinject, 1) + randn(numinject,1)*v_spread];
    p_vy = [p_vy; randn(numinject,1)*v_spread];
    p_isCEX = [p_isCEX; false(numinject, 1)];
    ...
    if chk_erosion.Value
        ...
        if any(is_cex_hit)
            ...
            % Keep erosion damage decoupled from the particle acceleration control.
            % Using ui_Accel here makes tiny damage increments look catastrophically large.
            erosionScale = 1e-6;
            damage_added = accumarray([iy, ix], ...
                Y_yield .* erosionScale, ...
                size(damage_map));
    ...
    if mod(iteration, 5) == 0
        ...
        if mod(iteration, 20) == 0
            axialSnapshot = computeAxialEnergyDiagnostic(80);
            axialEnergySnapshots{end+1} = axialSnapshot;
            axialDiagBinCenters = axialSnapshot.binCentersXmm;
        end
    end
end
...
function updateSimulationPlots(min_pot, current_div, angles, post_grid_mask, ...
    ...
    if any(cex_mask)
        set(h_cex, 'XData', p_x(cex_mask), 'YData', p_y(cex_mask), ...
            'CData', repmat([0.0 0.7 0.1], sum(cex_mask), 1), 'SizeData', 10);
    else
        set(h_cex, 'XData', NaN, 'YData', NaN, 'CData', [0.0 0.7 0.1]);
    end
...
function toggleSim(~,~)
    if sim.isSweepRunning
        ...
        btntoggle.Text = 'Stopping sweep...';
        btntoggle.BackgroundColor = [1 0.2 0.2];
        txtstatus.Text = 'Status: stop requested for sweep.';
        lblsweepStatus.Text = 'Sweep stop requested...';
        drawnow limitrate nocallbacks;
        return
    end
    ...
    if sim.isRunning
        ...
        sim.isRunning = false;
        btntoggle.Text = '2. START BEAM EXTRACTION';
        btntoggle.BackgroundColor = [0.8 1.0 0.8];
        txtstatus.Text = 'Status: beam paused.';
    else
        sim.stopRequested = false;
        sim.isRunning = true;
        btntoggle.Text = '2. STOP BEAM EXTRACTION';
        btntoggle.BackgroundColor = [1.0 0.8 0.8];
        txtstatus.Text = 'Status: beam extraction running.';
    end
...
% stack, resets simulation state, and solves the initial Laplace field.
    ...
    gridVoltages = [grid_defs.voltage];
    accelSpan_eV = max(gridVoltages) - min(gridVoltages);
    energy_max_eV = max(accelSpan_eV + 50, 50);
    clim(ax_live, [0 energy_max_eV]);
    ...
    for k = 1:numel(grid_defs)
        xstart = x_cursor;
        xend = xstart + grid_defs(k).thickness;
    
        gridXStart(k) = xstart;
        gridXEnd(k) = xend;
    
        ingrid = X >= xstart & X <= xend;
        Rgrid = grid_defs(k).holeRadius + max(0, X - xstart) * tand(grid_defs(k).chamfer);
        maskk = ingrid & (abs(Y - yCenter) >= Rgrid);
    
        isBound(maskk) = true;
        Vfixed(maskk) = grid_defs(k).voltage;
    
        x_cursor = xend + grid_defs(k).gapAfter;
    end
...
function recalcLaplace()
   ...
   for it = 1:maxIter
        ...
        Vtmp = V(2:end-1,2:end-1);
        freeInterior = ~isBound(2:end-1,2:end-1);
    
        Vtmp(freeInterior) = Vtmp(freeInterior) + ...
            omega * (Vnew(freeInterior) - Vtmp(freeInterior));
    
        V(2:end-1,2:end-1) = Vtmp;
        V(isBound) = V_fixed(isBound);
    ...
    [dVdy, dVdx] = gradient(V, dy*1e-3, dx*1e-3);
    Ex = -dVdx;
    Ey = -dVdy;
    ...
    yCenter = Ly/2;
    seedHalfSpan = max(0.2, grid_defs(1).holeRadius * 0.95);
    startY = linspace(max(0.05, yCenter - seedHalfSpan), ...
                      min(Ly - 0.05, yCenter + seedHalfSpan), nSeeds);
...
function [isOK, vmax_est, dl_min_m, ratio] = checkParticleStepLimit()
    ...
    gridVoltages = [grid_defs.voltage];
    accelSpan_eV = max(gridVoltages) - min(gridVoltages);
    energy_max_eV = max(accelSpan_eV, ui_Ti.Value);
...
function saveVelocityData(~, ~)
    ...
    if ~isequal(filename, 0)
        ...
        grid_defs_out = grid_defs;
        axialEnergySnapshotsOut = axialEnergySnapshots;
        axialDiagBinCentersOut = axialDiagBinCenters;
        ...
        save(fullfile(pathname, filename), ...
            ...
            'axialEnergySnapshotsOut', ...
            'axialDiagBinCentersOut', ...
            'grid_defs_out', ...
            'sim_params');
...
function snapshot = computeAxialEnergyDiagnostic(nBins)
    if nargin < 1
        nBins = 80;
    end
    ...
    snapshot = struct();
    snapshot.iteration = iteration;
    snapshot.binCentersXmm = [];
    snapshot.meanEnergyeV = [];
    snapshot.meanExVpm = [];
    snapshot.meanEmagVpm = [];
    snapshot.particleCount = [];
    snapshot.activePrimaryCount = 0;

    if isempty(p_x) || isempty(X) || isempty(Ex) || isempty(Ey)
        return;
    end
    ...
    xp = p_x(primaryMask);
    yp = p_y(primaryMask);
    vxp = p_vx(primaryMask);
    vyp = p_vy(primaryMask);

    KEeV = 0.5 * sim.m_XE * (vxp.^2 + vyp.^2) / sim.q;

    Ex_p = interp2(X, Y, Ex, xp, yp, 'linear', 0);
    Ey_p = interp2(X, Y, Ey, xp, yp, 'linear', 0);
    Emag_p = sqrt(Ex_p.^2 + Ey_p.^2);

    edges = linspace(0, Lx, nBins + 1);
    centers = 0.5 * (edges(1:end-1) + edges(2:end));

    meanEnergy = NaN(1, nBins);
    meanEx = NaN(1, nBins);
    meanEmag = NaN(1, nBins);
    counts = zeros(1, nBins);

    for i = 1:nBins
        inBin = (xp >= edges(i)) & (xp < edges(i+1));
        if i == nBins
            inBin = (xp >= edges(i)) & (xp <= edges(i+1));
        end
        ...
        if counts(i) > 0
            meanEnergy(i) = mean(KEeV(inBin));
            meanEx(i) = mean(Ex_p(inBin));
            meanEmag(i) = mean(Emag_p(inBin));
        end
    end

    snapshot.binCentersXmm = centers;
    snapshot.meanEnergyeV = meanEnergy;
    snapshot.meanExVpm = meanEx;
    snapshot.meanEmagVpm = meanEmag;
    snapshot.particleCount = counts;
    snapshot.activePrimaryCount = sum(primaryMask);
end
    function onClose(~,~)
    sim.isRunning = false;
    sim.stopSweepRequested = true;
    sim.isSweepRunning = false;
    sim.shouldClose = true;
